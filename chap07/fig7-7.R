...
p <- ggplot(data=data.frame(X=c(0,5)), aes(x=X)) +
  ...
  stat_function(fun=function(x) 1.8/(1+50*exp(-2*x)), aes(linetype='1'), size=1) +
  stat_function(fun=function(x) 8*(exp(-x) - exp(-2*x)), aes(linetype='2'), size=1) +
  scale_linetype_manual(values=c('solid', '52', '12')) +
  labs(linetype='Model', x='x', y='y')
