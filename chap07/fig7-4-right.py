...
fit = cmdstanpy.from_csv('output/result-model7-2')
mu_ms = fit.stan_variable(var='mu')
N_ms = len(mu_ms)
noise_ms = np.repeat(np.log10(d.Y.to_numpy())[None, :], N_ms, axis=0) - mu_ms
# noise_ms = np.einsum('n,m->mn', np.log10(d.Y), np.repeat(1, N_ms)) - mu_ms

mode_list = []
for i in range(N):
    density = stats.gaussian_kde(noise_ms[:,i])
    xs = np.linspace(-0.3, 0.4, 201)
    ys = density(xs)
    mode_i = np.argmax(ys)
    mode_x = xs[mode_i]
    mode_y = ys[mode_i]
...
p.save(filename='output/fig7-4-right.py.png', dpi=300, width=4, height=3)
