...
import numpy as np
import cmdstanpy
from plotnine import ggplot, aes, theme_bw, geom_ribbon, geom_line, geom_point, scale_x_continuous, ylim, labs

d = pandas.read_csv('input/data-conc.csv')
Tp = 60
Xp = np.linspace(0, 24, num=Tp)

fit = cmdstanpy.from_csv('output/result-model7-3')
...
p = (ggplot()
    ...
    + geom_point(d, aes('Time', 'Y'), shape='o', size=2)
    + scale_x_continuous(breaks=d.Time, limits=[0, 24])
    + ylim(-2.5, 16)
    + labs(x='Time (hour)', y='Y')
)
p.save(filename='output/fig7-5-right.py.png', dpi=300, width=4, height=3)
