...
p <- ggplot() +
  ...
  geom_line(data=d, aes(x=x, y=y1), color='black', size=1, linetype='31') +
  geom_line(data=d, aes(x=x, y=y2), color='black', size=1) +
  labs(x='y', y='density')
ggsave(file='output/fig9-2-left.png', plot=p, dpi=300, w=4, h=3)
...
ggsave(file='output/fig9-2-right.png', plot=p, dpi=300, w=4, h=3)
