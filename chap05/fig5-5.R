...
d <- read.csv(file='input/data-shopping-2.csv') %>% 
  mutate(Sex = factor(Sex), proportion = Y/M) %>% 
  select(-PersonID)
N_col <- ncol(d)
...
for (i in 1:N_col) {
  ...
  if (class(x) == 'factor') {
    p <- p + geom_bar(aes(fill=Sex), color='grey20')
  } else {
    bw <- (max(x)-min(x))/10
    p <- p + geom_histogram(aes(fill=Sex), color='grey20', binwidth=bw) +
...
png(file='output/fig5-5.png', w=2000, h=2000, res=300)
